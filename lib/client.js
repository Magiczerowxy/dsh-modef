window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// Local bindSnapshotSelector equivalent. The old shared package
		// (@deepseek-ai/dsh-client-web-react) no longer ships in current DSH
		// bundles (DSh-rc.2 / Desktop 2.0.x), so the plugin no longer depends
		// on it. React's native useSyncExternalStore provides the same
		// subscribe/getSnapshot selector contract; all call sites in this
		// plugin pass the identity selector, so no selector memoization is
		// needed (the snapshot reference is already stable between changes).
		const bindSnapshotSelector = (function () {
			// Per-source identity cache, mirroring the new official renderer's
			// observableHook: the subscribe/getSnapshot closures are captured
			// once per source so useSyncExternalStore never resubscribes.
			const cache = new WeakMap();
			return function (source) {
				let hook = cache.get(source);
				if (hook === void 0) {
					const subscribe = function (fn) { return source.subscribe(fn); };
					const getSnapshot = function () { return source.getSnapshot(); };
		...
		function ModelEffortControl(props) {
			...
			// Whether the pending close should refocus the trigger once the panel
			// is gone. Read at finish time (which can happen either from the
			// pop-out animationend or from the safety timer).
			...
			const cancelClose = function () {
				...
				// A cancelled close (reopen mid-transition) must restore the
				// panel's inline visuals, otherwise it stays invisible.
				if (panelRef.current) {
			...
			// The single place that actually tears the panel down. It is
			// reached as soon as the close transition has finished playing
			// (transitionend on opacity), or when the pop-out animation end
			// fires, or by a safety timer. All paths converge here, so the
			// teardown itself is idempotent.
			const finishCloseEffort = function () {
				console.info("[dms] close: finish (unmount)");
				if (closeTimerRef.current !== null) { clearTimeout(closeTimerRef.current); closeTimerRef.current = null; }
				...
				setEffortOpen(false);
				setEffortClosing(false);
				if (doRefocus) queueMicrotask(function () { if (effortTriggerRef.current) effortTriggerRef.current.focus(); });
			};
			// Plays the fade-out directly on the panel element through an
			// inline CSS transition. This deliberately does NOT depend on the
			// React `effortClosing` class render: some host environments fail
			// to apply the closing class in time, which would otherwise make
			// the panel vanish with no visual feedback at all. The unmount
			// happens on transitionend, with the timer as a safety net.
			const playCloseTransition = function (panelEl) {
				console.info("[dms] close: play inline transition");
				if (closeTransEndRef.current !== null) {
					panelEl.removeEventListener("transitionend", closeTransEndRef.current);
					closeTransEndRef.current = null;
				}
				const onTEnd = function (ev) {
				...
				// Commit current styles first, then change them, so the
				// browser interpolates instead of snapping.
			...
			const requestCloseEffort = function (refocus) {
				...
				console.info("[dms] close: request (closing render follows)", !!panelRef.current, !!refocus);
				...
				// Safety net: if neither the transition nor the pop-out
				// animation can finish (edge cases), tear down anyway.
				closeTimerRef.current = setTimeout(finishCloseEffort, delay);
			};
			const closeEffortNow = function () {
				console.info("[dms] close: instant (no transition)");
				cancelClose();
				closeRefocusRef.current = false;
				setEffortClosing(false);
				setEffortOpen(false);
			};
			// The panel's pop-out animation is the source of truth for unmount:
			// as soon as dms-pop-out finishes, tear the panel down. Child
			// element animation ends (sparks, shrink) bubble up but carry a
			// different animationName, so only the panel's own fade-out counts.
			const onPanelAnimationEnd = function (event) {
				if (effortClosing && event && event.animationName === "dms-pop-out") {
					finishCloseEffort();
				}
			};
			const commitEffort = function (idx, keepOpen) {
				...
				// Arm the undertow entry animation only when the user really
				// switches a tier INTO MAX. Switching between non-MAX tiers,
				// staying on MAX, or merely reopening the panel never arms it.
			...
			// The mode always follows the visual position (visualIdxRef), never
			// the async commit state: the moment the handle leaves MAX — while
			// dragging, during the snap animation, or before the model-directory
			// select settles — the undertow effect starts its exit transition
			// in lockstep with the thumb motion. There is no wait for the
			// animation to finish or the select to resolve, so the transition
			// never lags behind the handle.
			const effMode = stops.length > 1 && visIdx === maxIdx ? "max" : (stops.length > 1 && visIdx === maxIdx - 1 ? "high" : "base");
			...
			// Disarm the undertow reveal when the panel closes: the entry
			// animation is armed only by a real tier switch into MAX while
			// the panel is open, so a plain reopen must not replay it. Runs
			// as soon as effortOpen turns false, before the panel's closing
			// transition finishes.
			React.useEffect(function () {
			...
			// fading out. The reveal animation plays only when the panel is
			// open AND the user really switched a tier into MAX while it was
			// open (armed in commitEffort, disarmed when the panel closes);
			// reopening the panel on MAX shows the settled effect instantly.
		...
		function apply(ctx) {
			console.info("[dms] modef client apply (rc.2-compatible build)");
			injectStyles();
